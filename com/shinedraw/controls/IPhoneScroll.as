...
package com.shinedraw.controls {
	...
	public class IPhoneScroll {
		...
		private var _myScrollBar:DisplayObjectContainer;
		private var _stage:Stage;
		public var _started:Boolean =false;
		
		public function IPhoneScroll( pContent:DisplayObjectContainer, pStage:Stage, pScrollBar:DisplayObjectContainer ) {
			_stage = pStage;
			_myScrollElement = pContent;
			_myScrollBar = pScrollBar
			_canvasHeight = _myScrollElement.height;
			
			start();
			trace("hello scroller")
			// add handlers
			...
			_myScrollBar.addEventListener(MouseEvent.MOUSE_DOWN, on_mouse_down);
			_myScrollBar.addEventListener(Event.ENTER_FRAME, on_enter_frame);
		}
